...
function ToggleIcon({ open }: { open: boolean }) {
  return (
    <svg
      className={`h-[9px] w-[9px] flex-shrink-0 fill-[#8A9E94] transition-transform duration-150 ${open ? "rotate-90" : ""}`}
      viewBox="0 0 8 10"
    >
...
function CheckIcon() {
  return (
    <svg className="h-2.5 w-2.5 text-[#0D1F1A]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function SpinnerIcon({ className = "h-3 w-3" }: { className?: string }) {
  return (
    <svg className={`${className} animate-spin text-[#8A9E94]`} fill="none" viewBox="0 0 24 24">
...
const STATUS_STYLES: Record<string, string> = {
  "Received":               "bg-[#60A5FA]/15 text-[#60A5FA]",
  "Under Review":           "bg-[#F59E0B]/15 text-[#F59E0B]",
  "Approved":               "bg-[#4ADE80]/15 text-[#4ADE80]",
  "Approved with Comments": "bg-[#C9A84C]/15 text-[#C9A84C]",
  "Rejected":               "bg-[#F87171]/15 text-[#F87171]",
  "Revise and Resubmit":    "bg-[#F59E0B]/15 text-[#F59E0B]",
  "Needs Review":           "bg-[#F59E0B]/15 text-[#F59E0B]",
}
function StatusBadge({ status }: { status: string }) {
  const cls = STATUS_STYLES[status] ?? "bg-[#2A4F3F] text-[#8A9E94] border-[#2A4F3F]"
...
const RFI_STATUS_STYLES: Record<string, string> = {
  "Open":         "bg-[#60A5FA]/15 text-[#60A5FA]",
  "Under Review": "bg-[#F59E0B]/15 text-[#F59E0B]",
  "Answered":     "bg-[#C9A84C]/15 text-[#C9A84C]",
  "Closed":       "bg-[#4ADE80]/15 text-[#4ADE80]",
  "Void":         "bg-[#2A4F3F] text-[#8A9E94]",
}
function RfiStatusBadge({ status }: { status: string }) {
  const cls = RFI_STATUS_STYLES[status] ?? "bg-[#2A4F3F] text-[#8A9E94] border-[#2A4F3F]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
...
const PUNCH_PRIORITY_STYLES: Record<string, string> = {
  "Low":      "bg-[#2A4F3F] text-[#8A9E94]",
  "Medium":   "bg-[#C9A84C]/15 text-[#C9A84C]",
  "High":     "bg-[#F59E0B]/15 text-[#F59E0B]",
  "Critical": "bg-[#F87171]/15 text-[#F87171]",
}
function PunchStatusBadge({ status }: { status: string }) {
  const cls = PUNCH_STATUS_STYLES[status] ?? "bg-[#2A4F3F] text-[#8A9E94] border-[#2A4F3F]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
function PunchPriorityBadge({ priority }: { priority: string }) {
  const cls = PUNCH_PRIORITY_STYLES[priority] ?? "bg-[#2A4F3F] text-[#8A9E94] border-[#2A4F3F]"
...
const DRAWING_STATUS_STYLES: Record<string, string> = {
  "Issued for Construction": "bg-[#4ADE80]/15 text-[#4ADE80]",
  "Issued for Bid":          "bg-[#60A5FA]/15 text-[#60A5FA]",
  "Issued for Review":       "bg-[#F59E0B]/15 text-[#F59E0B]",
  "Record Drawings":         "bg-[#C9A84C]/15 text-[#C9A84C]",
  "Superseded":              "bg-[#2A4F3F] text-[#8A9E94]",
  "Void":                    "bg-[#F87171]/15 text-[#F87171]",
}
function DrawingStatusBadge({ status }: { status: string }) {
  const cls = DRAWING_STATUS_STYLES[status] ?? "bg-[#2A4F3F] text-[#8A9E94] border-[#2A4F3F]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
  ...
  return (
    <div ref={containerRef} className="relative">
      <input
        ...
        className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 focus:border-[#C9A84C]/50 placeholder:text-[#8A9E94] transition-all"
      />
      {open && filtered.length > 0 && (
        <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-[#132820] border border-[#2A4F3F] rounded-md shadow-xl max-h-44 overflow-y-auto">
          {filtered.map(opt => (
            <button
              key={opt}
              type="button"
              onMouseDown={e => { e.preventDefault(); onChange(opt); setOpen(false) }}
              className="w-full text-left px-3 py-1.5 text-[13px] text-[#F0EDE4] hover:bg-white/[0.07] transition-colors"
            >
              {opt}
            </button>
          ))}
...
function SidebarFileRow({ file, indent, onDelete, onOpen }: { file: SubmittalFile; indent: number; onDelete?: () => void; onOpen: () => void }) {
  ...
  return (
    <div
      className="group flex items-center gap-1.5 h-7 rounded-md hover:bg-[#F0EDE4]/[0.05] transition-colors cursor-pointer"
    ...
    >
      ...
      <span className="flex-1 min-w-0 text-[12px] text-[#8A9E94] truncate">{file.file_name}</span>
      {onDelete && (
        <button
          onClick={e => { e.stopPropagation(); onDelete() }}
          title="Delete file"
          className="opacity-0 group-hover:opacity-100 flex-shrink-0 text-[#8A9E94] hover:text-red-400 transition-all rounded p-0.5"
        >
...
export default function Home() {
  ...
  const inputCls = "w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[14px] text-[#F0EDE4] bg-[#1C3B2E] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 focus:border-[#C9A84C]/50 placeholder:text-[#8A9E94] transition-all"
  const labelCls = "block text-[11px] font-semibold text-[#8A9E94] uppercase tracking-[0.08em] mb-1.5"

  return (
    <div className="flex min-h-screen bg-[#0D1F1A]">

      {/* ── Sidebar ───────────────────────────────────────────────────────── */}
      {sidebarOpen && (
        <div className="fixed inset-0 z-30" onClick={() => { setSidebarOpen(false); sessionStorage.setItem("sidebarOpen", "false") }} />
      )}
      <aside className={`fixed left-0 top-0 h-screen z-40 bg-[#132820] border-r border-[#2A4F3F] flex flex-col overflow-hidden transition-[width] duration-200 ease-in-out ${sidebarOpen ? "w-80" : "w-12"}`}>

        {/* Rail header — always visible */}
        <div className="flex-shrink-0 flex items-center justify-between h-12 px-3 border-b border-[#2A4F3F]">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-6 h-6 rounded-md bg-[#C9A84C]/20 border border-[#C9A84C]/30 flex items-center justify-center flex-shrink-0">
              <svg className="w-3.5 h-3.5 text-[#C9A84C]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            {sidebarOpen && <span className="text-[14px] font-bold text-[#F0EDE4] tracking-tight truncate">TuttoHQ</span>}
          </div>
          <button
            onClick={() => { const next = !sidebarOpen; setSidebarOpen(next); sessionStorage.setItem("sidebarOpen", String(next)) }}
            className="w-5 h-5 flex items-center justify-center text-[#8A9E94] hover:text-[#8A9E94] transition-colors flex-shrink-0 rounded hover:bg-[#F0EDE4]/[0.05]"
          >
        ...
        {sidebarOpen && <><p className="text-[11px] text-[#8A9E94] px-5 pt-2 pb-1">Construction Documents</p>

        {/* Search */}
        <div className="flex-shrink-0 px-3 pb-2">
          <form onSubmit={handleSearch}>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 flex items-center pl-2.5 pointer-events-none text-[#8A9E94]">
                <SearchIcon />
              </div>
              <input
                ref={inputRef}
                ...
                className="w-full h-8 pl-8 pr-6 rounded-md text-[13px] bg-[#0D1F1A] border border-[#2A4F3F] text-[#F0EDE4] placeholder-[#8A9E94] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 focus:border-[#C9A84C]/50 transition-all"
              />
              {query && (
                <button
                  type="button"
                  onClick={clearSearch}
                  className="absolute inset-y-0 right-0 flex items-center pr-2 text-[#8A9E94] hover:text-[#8A9E94] transition-colors"
                >
                  <XIcon />
                </button>
              )}
        ...
        <div className="flex-shrink-0 border-t border-[#2A4F3F] mx-3 mt-0.5 mb-1.5" />

        {/* Section label + upload button */}
        <div className="flex-shrink-0 px-4 pb-1">
          {isSearchMode && searchAiSummary && !searching && (
            <p className="text-[11px] text-[#C9A84C] mb-1 flex items-center gap-1">
        ...
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">
            {isSearchMode
          ...
          <div className="flex items-center gap-2">
            {isSearchMode && !searching && (
              <button
                onClick={clearSearch}
                className="text-[11px] text-[#8A9E94] hover:text-[#F0EDE4] transition-colors"
              >
                Clear
              </button>
            )}
            {!isSearchMode && (
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors"
                >
                  <SlidersIcon />
                </button>
                <button
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2 min-h-0">
          ...
          {treeLoading && !isSearchMode && (
            <div className="flex items-center gap-2 px-3 py-2 text-[13px] text-[#8A9E94]">
              <SpinnerIcon /> Loading…
            </div>
          )}
          ...
          {!searching && isSearchMode && (
            <>
              ...
              {searchResults?.length === 0 && (
                <p className="px-3 py-2 text-[13px] text-[#8A9E94]">No results for &ldquo;{query}&rdquo;</p>
              )}
            ...
            return (
              <div key={div.num}>
                <button
                  ...
                  className="w-full flex items-center gap-1.5 h-8 px-2 rounded-md hover:bg-[#F0EDE4]/[0.05] transition-colors text-left group"
                >
                  <span className="w-4 flex items-center justify-center flex-shrink-0">
                    <ToggleIcon open={isOpen} />
                  </span>
                  <span className="text-[11px] font-mono text-[#8A9E94] w-5 text-right flex-shrink-0">{div.num}</span>
                  <span className="flex-1 text-[13px] font-semibold text-[#F0EDE4] truncate">{div.name}</span>
                  {div.file_count > 0 && (
                    <span className="text-[10px] text-[#8A9E94] flex-shrink-0 tabular-nums bg-[#C9A84C]/10 px-1.5 py-0.5 rounded">{div.file_count}</span>
                  )}
                </button>

                <div className={`grid transition-all duration-150 ${isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                  <div className="overflow-hidden">
                    <div className="ml-[20px] border-l border-[#2A4F3F] pl-1">
                        ...
                        return (
                          <div key={sec.code}>
                            <button
                              ...
                              className="w-full flex items-center gap-1.5 h-7 px-1.5 rounded-md hover:bg-[#F0EDE4]/[0.04] transition-colors text-left group"
                            >
                              ...
                              <span className="flex-1 text-[12px] text-[#8A9E94] truncate">
                                <span className="font-mono text-[#8A9E94] mr-1.5">{sec.code}</span>{sec.name}
                              </span>
                              {(sec.file_count ?? 0) > 0 && (
                                <span className="text-[10px] text-[#8A9E94] flex-shrink-0 tabular-nums bg-[#C9A84C]/10 px-1.5 py-0.5 rounded">{sec.file_count}</span>
                              )}
                            </button>

                            <div className={`grid transition-all duration-150 ${secOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                              <div className="overflow-hidden">
                                <div className="ml-[14px] border-l border-[#2A4F3F] pl-1">
                                  {secLoading && (
                                    <div className="flex items-center gap-1.5 h-7 px-2 text-[12px] text-[#8A9E94]">
                                      <SpinnerIcon className="h-2.5 w-2.5" /> Loading…
                                    </div>
                                  )}
                                  {!secLoading && sectionFiles[sec.code] !== undefined && files.length === 0 && (
                                    <p className="px-2 h-7 flex items-center text-[12px] text-[#8A9E94]">Empty</p>
                                  )}
        ...
        {/* Settings + sign out */}
        <div className="flex-shrink-0 border-t border-[#2A4F3F]">
          <div className="px-2 pt-1.5">
            <Link
              href="/settings"
              className="flex items-center gap-2 h-8 px-2 rounded-md text-[12px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] hover:text-[#F0EDE4] transition-colors"
            >
          ...
          <div className="flex items-center justify-between px-3 py-2.5">
            <span className="text-[11px] text-[#8A9E94] truncate min-w-0">{userEmail}</span>
            <button
              onClick={signOut}
              className="text-[11px] text-[#8A9E94] hover:text-[#F0EDE4] transition-colors flex-shrink-0 ml-2"
            >
              Sign out
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 ml-12">
        ...
        {/* Logo bar */}
        {logoUrl && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] flex items-center justify-end px-6 py-2 bg-[#132820]">
            <img src={logoUrl} alt="Company logo" className="h-7 max-w-[160px] object-contain" />
          </div>
        )}

        {/* Module navigation */}
        <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#0D1F1A] flex items-center px-4 gap-0.5">
          {(["submittals","rfis","changeorders","punch","daily","drawings","closeout"] as const).map(mod => {
            ...
            return (
              <button key={mod} onClick={() => setActiveModule(mod)}
                className={`px-3 py-3 text-[13px] font-medium border-b-2 transition-colors whitespace-nowrap ${isActive ? "border-[#C9A84C] text-[#F0EDE4]" : "border-transparent text-[#8A9E94] hover:text-[#8A9E94]"}`}>
                {labels[mod]}
          ...
          {appProjects.length > 0 && (
            <div className="ml-auto flex items-center gap-2 py-1.5 flex-shrink-0">
              <span className="text-[11px] text-[#8A9E94] whitespace-nowrap">Project:</span>
              <div className="relative">
                <select
                  value={globalProjectId}
                  onChange={e => setGlobalProjectId(e.target.value)}
                  className="h-7 pl-3 pr-7 rounded-md border border-[#2A4F3F] bg-[#1C3B2E] text-[12px] text-[#F0EDE4] appearance-none cursor-pointer hover:border-[#2A4F3F] transition-colors focus:outline-none focus:border-[#C9A84C]"
                >
                ...
                <span className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[#8A9E94]">
              ...
              {globalProjectId && (
                <button onClick={() => setGlobalProjectId("")} className="text-[11px] text-[#8A9E94] hover:text-[#8A9E94] transition-colors px-1" title="Clear filter">✕</button>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
          <p className="text-[13px] font-semibold text-[#F0EDE4]">Submittal Log <span className="text-[#8A9E94] font-normal ml-1">({logSubmittals.length})</span></p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => { setShowBatch(true); setBatchPhase("select"); setBatchItems([]) }}
              className="h-8 px-3 rounded-md border border-[#2A4F3F] text-[12px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors flex items-center gap-1.5"
            >
              <LayersIcon /> Batch
            </button>
            <button
              onClick={() => setShowUpload(true)}
              className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5"
            >
              <PlusIcon /> Upload
            </button>
          </div>
        ...
        {activeModule === "rfis" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <p className="text-[13px] font-semibold text-[#F0EDE4]">RFI Log <span className="text-[#8A9E94] font-normal ml-1">({rfis.length})</span></p>
            <button onClick={() => setShowNewRfi(true)} className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5">
              <PlusIcon /> New RFI
            </button>
          </div>
        )}
        ...
        {activeModule === "changeorders" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <p className="text-[13px] font-semibold text-[#F0EDE4]">Change Orders <span className="text-[#8A9E94] font-normal ml-1">({changeOrders.length})</span></p>
            <button onClick={() => setShowNewCo(true)} className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5">
              <PlusIcon /> New CO
            </button>
          </div>
        )}
        ...
        {activeModule === "punch" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <div className="flex items-center gap-3">
              <p className="text-[13px] font-semibold text-[#F0EDE4]">Punch List <span className="text-[#8A9E94] font-normal ml-1">({punchItems.filter(p => p.status !== "Void").length} items)</span></p>
            ...
            <button onClick={() => setShowNewPunch(true)} className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5">
              <PlusIcon /> New Item
            </button>
          </div>
        )}
        ...
        {activeModule === "daily" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <p className="text-[13px] font-semibold text-[#F0EDE4]">Daily Reports <span className="text-[#8A9E94] font-normal ml-1">({dailyReports.length})</span></p>
            <button onClick={() => setShowNewDaily(true)} className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5">
              <PlusIcon /> New Report
            </button>
          </div>
        )}
        ...
        {activeModule === "drawings" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <p className="text-[13px] font-semibold text-[#F0EDE4]">Drawing Log <span className="text-[#8A9E94] font-normal ml-1">({drawings.filter(d => d.is_current).length} sheets)</span></p>
            <button onClick={() => { setShowNewDrawing(true); setAddRevisionFor(null); resetDwgForm() }} className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5">
        ...
        {activeModule === "closeout" && (
          <div className="flex-shrink-0 border-b border-[#2A4F3F] bg-[#132820] flex items-center justify-between px-4 py-2.5">
            <div className="flex items-center gap-3">
              <p className="text-[13px] font-semibold text-[#F0EDE4]">Project Closeout</p>
              {globalProjectId && closeoutItems.length > 0 && (() => {
                ...
                return <span className="text-[11px] text-[#8A9E94]">{pct}% complete</span>
              })()}
            </div>
            <div className="flex items-center gap-2">
              {globalProjectId && closeoutItems.length > 0 && (
                <button
                  ...
                  className="h-8 px-3 rounded-md border border-[#2A4F3F] text-[#8A9E94] text-[12px] font-semibold hover:border-[#2A4F3F] hover:text-[#F0EDE4] transition-colors flex items-center gap-1.5"
                >
                  <PlusIcon /> Add Item
                </button>
              )}
              {globalProjectId && closeoutItems.length > 0 && (
                <button
                  ...
                  className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors flex items-center gap-1.5 disabled:opacity-50"
                >
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {activeModule === "submittals" && (<>
          {logLoading ? (
            <div className="flex items-center justify-center h-40 gap-2 text-[13px] text-[#8A9E94]">
            ...
            <div className="flex flex-col items-center justify-center h-full py-24 text-center">
              ...
              <p className="text-[15px] font-bold text-[#F0EDE4]">No submittals yet</p>
              <p className="text-[13px] text-[#8A9E94] mt-1.5">Upload your first submittal to get started.</p>
              <button onClick={() => setShowUpload(true)} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                <PlusIcon /> Upload submittal
              </button>
            </div>
          ) : (
            <div className="mx-4 my-4 rounded-xl border border-[#2A4F3F] overflow-clip bg-[#1C3B2E]">
            <table className="w-full text-[13px] border-collapse">
              <thead className="sticky top-0 bg-[#132820] z-10">
                <tr className="border-b border-[#2A4F3F]">
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-10">#</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Title</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-32">Division</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-48">Section</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Date</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-36">Status</th>
                  <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-40">Actions</th>
                </tr>
              </thead>
              <tbody>
                {logSubmittals.map((s, i) => (
                  <tr key={s.id} className="border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors group">
                    <td className="px-4 py-2.5 text-[#8A9E94] tabular-nums text-[12px]">{logSubmittals.length - i}</td>
                    <td className="px-4 py-2.5 max-w-0">
                      <div className="flex items-center gap-1.5">
                        <p className="text-[#F0EDE4] font-medium truncate" title={s.file_name}>{s.file_name}</p>
                        {s.sender_email && (
                          <span title={`Received from ${s.sender_email}`} className="flex-shrink-0 text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {s.manually_overridden && (
                          <span className="text-[10px] text-[#C9A84C]">✎ Overridden</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] whitespace-nowrap">
                      {s.csi_division && <span className="font-mono text-[#8A9E94] mr-1">{s.csi_division}</span>}
                      {s.division_name}
                    </td>
                    <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{s.section_name ?? s.csi_section ?? "—"}</td>
                    <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] whitespace-nowrap">{fmtDate(s.received_at ?? s.created_at)}</td>
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="text-[11px] text-[#8A9E94] hover:text-[#F0EDE4] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors"
                        >Open</button>
                        <button
                        ...
                        {s.project_id ? (
                          <button
                            ...
                            className="text-[11px] text-[#C9A84C] hover:text-[#D4B56A] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors"
                          >Cover Sheet</button>
                        ) : (
                          <button
                        ...
                        <button
                          ...
                          className="text-[11px] text-[#8A9E94] hover:text-red-400 px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors"
          ...
          {activeModule === "rfis" && (
            rfisLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                ...
                <p className="text-[15px] font-bold text-[#F0EDE4]">No RFIs yet</p>
                <p className="text-[13px] text-[#8A9E94] mt-1.5">Create your first RFI to track questions and responses.</p>
                <button onClick={() => setShowNewRfi(true)} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                  <PlusIcon /> New RFI
                </button>
              </div>
            ) : (
            ...
            <table className="w-full text-[13px] border-collapse">
                <thead className="sticky top-0 bg-[#132820] z-10">
                  <tr className="border-b border-[#2A4F3F]">
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">RFI #</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Subject</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-32">Received From</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Spec Section</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Sched.</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Cost</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Due</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Status</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {rfis.map(r => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors">
                        <td className="px-4 py-2.5 text-[12px] font-mono text-[#C9A84C]">{r.rfi_number}</td>
                        <td className="px-4 py-2.5 max-w-0">
                          <p className="text-[#F0EDE4] font-medium truncate" title={r.subject}>{r.subject}</p>
                          {r.description && <p className="text-[11px] text-[#8A9E94] truncate">{r.description}</p>}
                        </td>
                        <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] truncate">{r.received_from ?? r.submitted_by ?? "—"}</td>
                        <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] font-mono">{r.specification_section ?? "—"}</td>
                        <td className="px-4 py-2.5 text-[12px]">
                          <span className={r.schedule_impact === "Yes" ? "text-amber-400" : r.schedule_impact === "No" ? "text-green-400" : "text-[#8A9E94]"}>{r.schedule_impact ?? "TBD"}</span>
                        </td>
                        <td className="px-4 py-2.5 text-[12px]">
                          <span className={r.cost_impact === "Yes" ? "text-amber-400" : r.cost_impact === "No" ? "text-green-400" : "text-[#8A9E94]"}>{r.cost_impact ?? "TBD"}</span>
                        </td>
                        <td className="px-4 py-2.5 text-[12px] whitespace-nowrap">
                          {r.due_date ? <span className={isOverdue ? "text-red-400 font-medium" : "text-[#8A9E94]"}>{fmtDateOnly(r.due_date)}{isOverdue ? " ⚠" : ""}</span> : <span className="text-[#8A9E94]">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1">
                            <button onClick={() => { setViewRfi(r); setRfiResponse(r.response ?? ""); setRfiResponseStatus(r.status) }}
                              className="text-[11px] text-[#8A9E94] hover:text-[#F0EDE4] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors">View</button>
                            <button onClick={() => generateRfiPdf(r.id)} disabled={rfiGeneratingPdf}
                              className="text-[11px] text-[#C9A84C] hover:text-[#D4B56A] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors disabled:opacity-50">PDF</button>
                            <button onClick={() => deleteRfi(r.id)}
                              className="text-[11px] text-red-400/60 hover:text-red-400 px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors">Del</button>
                          </div>
                        </td>
                      </tr>
                    )
          ...
          {activeModule === "changeorders" && (
            coLoading ? (
              ...
              <div className="flex flex-col min-h-full">
                ...
                {changeOrders.length > 0 && (() => {
                  ...
                  return (
                    <div className="flex items-stretch gap-3 px-4 py-3 border-b border-[#2A4F3F] flex-shrink-0">
                      {[
                        ...
                        <div key={label} className="flex-1 rounded-lg bg-[#1C3B2E] border border-[#2A4F3F] px-3 py-2">
                          <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-1">{label}</p>
                          <p className={`text-[15px] font-bold tabular-nums ${green ? "text-green-400" : "text-[#F0EDE4]"}`}>{value}</p>
                        </div>
                      ))}
                    </div>
                  )
                ...
                {changeOrders.length === 0 ? (
                  <div className="flex flex-col items-center justify-center flex-1 py-24 text-center">
                    ...
                    <p className="text-[15px] font-bold text-[#F0EDE4]">No change orders yet</p>
                    <p className="text-[13px] text-[#8A9E94] mt-1.5">Create your first change order to track scope changes.</p>
                    <button onClick={() => setShowNewCo(true)} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                      <PlusIcon /> New CO
                    </button>
                  </div>
                ) : (
            ...
            <table className="w-full text-[13px] border-collapse">
                    <thead className="sticky top-0 bg-[#132820] z-10">
                      <tr className="border-b border-[#2A4F3F]">
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">CO #</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-32">Project</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Proposal</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Pricing</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Sched.</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Date</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Status</th>
                        <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Actions</th>
                    ...
                    <tbody>
                      {changeOrders.map(c => {
                        ...
                        const statusColor: Record<string, string> = {
                          Draft: "bg-[#2A4F3F] text-[#8A9E94]",
                          Submitted: "bg-[#C9A84C]/15 text-[#C9A84C]",
                          "Under Review": "bg-[#F59E0B]/15 text-[#F59E0B]",
                          Approved: "bg-[#4ADE80]/15 text-[#4ADE80]",
                          Rejected: "bg-[#F87171]/15 text-[#F87171]",
                          Void: "bg-[#1C3B2E] text-[#8A9E94]",
                        }
                        const badgeCls = statusColor[c.status] ?? "bg-[#2A4F3F] text-[#8A9E94]"
                        return (
                          <tr key={c.id} className="border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors">
                            <td className="px-4 py-2.5 text-[12px] font-mono text-[#C9A84C]">{c.co_number}</td>
                            <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] truncate">{proj?.name ?? "—"}</td>
                            <td className="px-4 py-2.5 max-w-0"><p className="text-[#F0EDE4] truncate">{c.proposal ?? "—"}</p></td>
                            <td className="px-4 py-2.5 text-[#F0EDE4] text-[12px] tabular-nums font-medium">
                              {c.pricing_sum != null ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(c.pricing_sum) : "—"}
                            </td>
                            <td className="px-4 py-2.5 text-[12px]">
                              <span className={c.schedule_impact === "Yes" ? "text-amber-400" : c.schedule_impact === "No" ? "text-green-400" : "text-[#8A9E94]"}>{c.schedule_impact ?? "TBD"}</span>
                            </td>
                            <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] whitespace-nowrap">{c.date ? fmtDateOnly(c.date) : "—"}</td>
                            <td className="px-4 py-2.5">
          ...
          {activeModule === "punch" && (
            punchLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                ...
                <p className="text-[15px] font-bold text-[#F0EDE4]">No punch items yet</p>
                <p className="text-[13px] text-[#8A9E94] mt-1.5">Add items to track deficiencies and corrections.</p>
                <button onClick={() => setShowNewPunch(true)} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                  <PlusIcon /> New Item
                </button>
              </div>
            ) : (
            ...
            <table className="w-full text-[13px] border-collapse">
                <thead className="sticky top-0 bg-[#132820] z-10">
                  <tr className="border-b border-[#2A4F3F]">
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-10">#</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Item</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Description</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-32">Location</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-32">Assigned To</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Due</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Priority</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Status</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-16">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {punchItems.map((p, i) => {
                    ...
                    return (
                      <tr key={p.id} className={`border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors ${isStruck ? "opacity-50" : ""}`}>
                        <td className="px-4 py-2.5 text-[#8A9E94] tabular-nums text-[12px]">{punchItems.length - i}</td>
                        <td className="px-4 py-2.5 text-[12px] font-mono text-[#C9A84C]">{p.item_number}</td>
                        <td className="px-4 py-2.5 max-w-0">
                          <p className={`font-medium truncate ${isStruck ? "line-through text-[#8A9E94]" : "text-[#F0EDE4]"}`} title={p.description}>{p.description}</p>
                        </td>
                        <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{p.location ?? "—"}</td>
                        <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{p.assigned_to ?? "—"}</td>
                        <td className="px-4 py-2.5 text-[12px] whitespace-nowrap">
                          {p.due_date
                            ? <span className={isOverdue ? "text-red-400 font-medium" : "text-[#8A9E94]"}>{fmtDateOnly(p.due_date)}{isOverdue ? " ⚠" : ""}</span>
                            : <span className="text-[#8A9E94]">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-2.5">
                          ...
                          <button onClick={e => { e.stopPropagation(); deletePunchItem(p.id) }}
                            className="text-[11px] text-red-400 hover:text-red-300 px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors">Del</button>
                        </td>
                      </tr>
                    )
                  })}
          ...
          {activeModule === "daily" && (
            dailyLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                ...
                <p className="text-[15px] font-bold text-[#F0EDE4]">No daily reports yet</p>
                <p className="text-[13px] text-[#8A9E94] mt-1.5">Log daily site activity, weather, and manpower.</p>
                <button onClick={() => setShowNewDaily(true)} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                  <PlusIcon /> New Report
                </button>
              </div>
            ) : (
            ...
            <table className="w-full text-[13px] border-collapse">
                <thead className="sticky top-0 bg-[#132820] z-10">
                  <tr className="border-b border-[#2A4F3F]">
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-10">#</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Date</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Work Performed</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Prepared By</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Weather</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Manpower</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-20">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {dailyReports.map((r, i) => (
                    <tr key={r.id} className="border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors cursor-pointer" onClick={() => { setViewDaily(r); setDailyEditing(false) }}>
                      <td className="px-4 py-2.5 text-[#8A9E94] tabular-nums text-[12px]">{dailyReports.length - i}</td>
                      <td className="px-4 py-2.5 text-[#F0EDE4] font-medium text-[12px] whitespace-nowrap">{fmtDateOnly(r.report_date)}</td>
                      <td className="px-4 py-2.5 max-w-0">
                        <p className="text-[#8A9E94] text-[12px] truncate">{r.work_performed ?? <span className="text-[#8A9E94] italic">No description</span>}</p>
                      </td>
                      <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{r.prepared_by ?? "—"}</td>
                      <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{r.weather_conditions ?? "—"}{r.temperature ? ` · ${r.temperature}` : ""}</td>
                      <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] text-center">{r.manpower_count ?? "—"}</td>
                      <td className="px-4 py-2.5">
          ...
          {activeModule === "drawings" && (() => {
            ...
            return drawingsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                ...
                <p className="text-[15px] font-bold text-[#F0EDE4]">No drawings yet</p>
                <p className="text-[13px] text-[#8A9E94] mt-1.5">Add drawings to track revisions and status.</p>
                <button onClick={() => { setShowNewDrawing(true); setAddRevisionFor(null); resetDwgForm() }} className="mt-5 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                  <PlusIcon /> Add Drawing
                </button>
              </div>
            ) : (
            ...
            <table className="w-full text-[13px] border-collapse">
                <thead className="sticky top-0 bg-[#132820] z-10">
                  <tr className="border-b border-[#2A4F3F]">
                    <th className="w-8" />
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Drawing No.</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Sheet Title</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Discipline</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-16">Rev</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-24">Rev Date</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-44">Status</th>
                    <th className="text-left px-4 py-2.5 text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest w-28">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {currentDrawings.map(d => {
                    ...
                    return (
                      <>
                        <tr key={d.id} className="border-b border-[#2A4F3F]/60 hover:bg-[#C9A84C]/[0.05] transition-colors">
                          <td className="px-2 py-2.5 text-center">
                            {history.length > 0 && (
                              <button onClick={() => setExpandedDrawings(prev => { const n = new Set(prev); isExpanded ? n.delete(d.drawing_number) : n.add(d.drawing_number); return n })}
                                className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors flex items-center justify-center w-full" title={`${history.length} previous revision${history.length !== 1 ? "s" : ""}`}>
                                <ToggleIcon open={isExpanded} />
                              </button>
                            )}
                          </td>
                          <td className="px-4 py-2.5 text-[12px] font-mono text-[#C9A84C] whitespace-nowrap">{d.drawing_number}</td>
                          <td className="px-4 py-2.5 max-w-0"><p className="text-[#F0EDE4] font-medium truncate" title={d.sheet_title}>{d.sheet_title}</p></td>
                          <td className="px-4 py-2.5 text-[#8A9E94] text-[12px]">{d.discipline ?? "—"}</td>
                          <td className="px-4 py-2.5 text-[12px] font-mono font-bold text-[#F0EDE4]">{d.revision}</td>
                          <td className="px-4 py-2.5 text-[#8A9E94] text-[12px] whitespace-nowrap">{d.revision_date ? fmtDateOnly(d.revision_date) : "—"}</td>
                          <td className="px-4 py-2.5"><DrawingStatusBadge status={d.status} /></td>
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-1">
                              <button onClick={() => openAddRevision(d)}
                                className="text-[11px] text-[#C9A84C] hover:text-[#D4B56A] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors whitespace-nowrap">
                                + Rev
                              </button>
                        ...
                        {isExpanded && history.map(h => (
                          <tr key={h.id} className="border-b border-[#2A4F3F]/20 bg-[#0D1F1A]/40">
                            <td />
                            <td className="px-4 py-1.5 text-[11px] font-mono text-[#8A9E94]">{h.drawing_number}</td>
                            <td className="px-4 py-1.5 text-[11px] text-[#8A9E94] truncate max-w-0">{h.sheet_title}</td>
                            <td className="px-4 py-1.5 text-[11px] text-[#8A9E94]">{h.discipline ?? "—"}</td>
                            <td className="px-4 py-1.5 text-[11px] font-mono text-[#8A9E94]">{h.revision}</td>
                            <td className="px-4 py-1.5 text-[11px] text-[#8A9E94] whitespace-nowrap">{h.revision_date ? fmtDateOnly(h.revision_date) : "—"}</td>
                            <td className="px-4 py-1.5"><span className="text-[10px] text-[#8A9E94]">Superseded {h.superseded_at ? fmtDate(h.superseded_at) : ""}</span></td>
                            <td />
                          </tr>
                        ))}
                      </>
            ...
            return (
              <>
                ...
                {!globalProjectId ? (
                  <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                    <div className="w-14 h-14 rounded-2xl bg-[#C9A84C]/10 border border-[#C9A84C]/20 flex items-center justify-center mb-4">
                      <svg className="w-7 h-7 text-[#C9A84C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
                    </div>
                    <p className="text-[15px] font-bold text-[#F0EDE4]">Select a project to view closeout</p>
                    <p className="text-[13px] text-[#8A9E94] mt-1.5">Use the Project filter above to choose a project.</p>
                  </div>
                ) : closeoutLoading ? (
                  <div className="flex items-center justify-center h-40 gap-2 text-[13px] text-[#8A9E94]">
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-24 text-center">
                    ...
                    <p className="text-[15px] font-bold text-[#F0EDE4]">Closeout not started</p>
                    <p className="text-[13px] text-[#8A9E94] mt-1.5 max-w-xs">Initialize the closeout checklist to start tracking documents, inspections, and handover items for this project.</p>
                    <button
                      ...
                      className="mt-6 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2 disabled:opacity-50"
                    >
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {openPunchCount > 0 && (
                      <div className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-red-500/10 border border-red-500/30">
                        ...
                        <button onClick={() => setActiveModule("punch")} className="text-[11px] text-[#C9A84C] hover:text-[#D4B56A] transition-colors flex-shrink-0">Go to Punch →</button>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      {CATS.map(cat => {
                        ...
                        return (
                          <div key={cat.key} className="bg-[#132820] rounded-xl border border-[#2A4F3F] p-3">
                            <div className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-wide mb-2">{cat.label}</div>
                            <div className="text-[26px] font-extrabold leading-none mb-1" style={{ color }}>{pct}%</div>
                            <div className="text-[10px] text-[#8A9E94] mb-2">{cat.done}/{cat.total}</div>
                            <div className="h-1 rounded-full bg-[#2A4F3F] overflow-hidden">
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] overflow-hidden">
                          <div className="flex items-center justify-between px-4 py-3 border-b border-[#2A4F3F]">
                            <div className="flex items-center gap-2">
                              <span className="text-[13px] font-bold text-[#F0EDE4]">Documents</span>
                              <span className="text-[11px] text-[#8A9E94]">{docDone}/{docTotal} complete</span>
                            </div>
                            <button onClick={() => { setNewCloseoutCategory("documents"); setShowNewCloseout(true) }} className="text-[11px] text-[#8A9E94] hover:text-[#8A9E94] transition-colors flex items-center gap-1"><PlusIcon /> Add</button>
                          </div>

                          {/* Submittals sub-section */}
                          {closeoutAllSubmittals.length > 0 && (
                            <div className="border-b border-[#2A4F3F]/40">
                              <div className="flex items-center justify-between px-4 py-2 bg-[#0D1F1A]/30">
                                <span className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-wider">Submittals ({approvedSubs}/{closeoutAllSubmittals.length} Approved)</span>
                                <button onClick={() => setActiveModule("submittals")} className="text-[10px] text-[#C9A84C] hover:text-[#D4B56A]">View all →</button>
                              </div>
                              {closeoutAllSubmittals.map(s => {
                                const done = s.review_status === "Approved"
                                return (
                                  <div key={s.id} className="flex items-center gap-3 px-4 py-2 border-b border-[#2A4F3F]/20 last:border-0 hover:bg-[#F0EDE4]/[0.01]">
                                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: done ? "#10b981" : "#ef4444" }} />
                                    <span className="text-[11px] text-[#8A9E94] font-mono flex-shrink-0 w-16 truncate">{s.csi_section ?? s.csi_division ?? "—"}</span>
                                    <span className={`flex-1 text-[12px] truncate ${done ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{s.file_name}</span>
                          ...
                          {closeoutAllRFIs.length > 0 && (
                            <div className="border-b border-[#2A4F3F]/40">
                              <div className="flex items-center justify-between px-4 py-2 bg-[#0D1F1A]/30">
                                <span className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-wider">RFIs ({resolvedRFIs}/{closeoutAllRFIs.length} Resolved)</span>
                                <button onClick={() => setActiveModule("rfis")} className="text-[10px] text-[#C9A84C] hover:text-[#D4B56A]">View all →</button>
                              </div>
                              {closeoutAllRFIs.map(r => {
                                const done = r.status === "Closed"
                                return (
                                  <div key={r.id} className="flex items-center gap-3 px-4 py-2 border-b border-[#2A4F3F]/20 last:border-0 hover:bg-[#F0EDE4]/[0.01]">
                                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: done ? "#10b981" : "#f59e0b" }} />
                                    <span className="text-[11px] text-[#8A9E94] font-mono flex-shrink-0">{r.rfi_number}</span>
                                    <span className={`flex-1 text-[12px] truncate ${done ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{r.subject}</span>
                          ...
                          {closeoutAllCOs.length > 0 && (
                            <div className="border-b border-[#2A4F3F]/40">
                              <div className="flex items-center justify-between px-4 py-2 bg-[#0D1F1A]/30">
                                <span className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-wider">Change Orders ({approvedCOs}/{closeoutAllCOs.length} Signed)</span>
                                <button onClick={() => setActiveModule("changeorders")} className="text-[10px] text-[#C9A84C] hover:text-[#D4B56A]">View all →</button>
                              </div>
                              {closeoutAllCOs.map(c => {
                                const done = c.status === "Approved"
                                return (
                                  <div key={c.id} className="flex items-center gap-3 px-4 py-2 border-b border-[#2A4F3F]/20 last:border-0 hover:bg-[#F0EDE4]/[0.01]">
                                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: done ? "#10b981" : "#ef4444" }} />
                                    <span className="text-[11px] text-[#8A9E94] font-mono flex-shrink-0">CO-{c.co_number}</span>
                                    <span className={`flex-1 text-[12px] truncate ${done ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{c.proposal ?? "—"}</span>
                          ...
                          {closeoutAllDrawings.length > 0 && (
                            <div className="border-b border-[#2A4F3F]/40">
                              <div className="flex items-center justify-between px-4 py-2 bg-[#0D1F1A]/30">
                                <span className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-wider">Drawings — As-Built ({asBuiltDwgs}/{closeoutAllDrawings.length} Confirmed)</span>
                                <button onClick={() => setActiveModule("drawings")} className="text-[10px] text-[#C9A84C] hover:text-[#D4B56A]">View all →</button>
                              </div>
                              {closeoutAllDrawings.map(d => {
                                const done = d.status === "As-Built"
                                return (
                                  <div key={d.id} className="flex items-center gap-3 px-4 py-2 border-b border-[#2A4F3F]/20 last:border-0 hover:bg-[#F0EDE4]/[0.01]">
                                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: done ? "#10b981" : "#f59e0b" }} />
                                    <span className="text-[11px] text-[#8A9E94] font-mono flex-shrink-0 w-16 truncate">{d.drawing_number}</span>
                                    <span className={`flex-1 text-[12px] truncate ${done ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{d.sheet_title}</span>
                          ...
                          {storedDocs.map(item => {
                            ...
                            return (
                              <div key={item.id} className="border-b border-[#2A4F3F]/40 last:border-0">
                                <div className={`flex items-center gap-3 px-4 py-3 hover:bg-[#F0EDE4]/[0.02] transition-colors ${isEditing ? "bg-[#F0EDE4]/[0.02]" : ""}`}>
                                  ...
                                  {typeLabel && <span className="text-[10px] font-bold text-[#8A9E94] bg-[#1C3B2E] px-1.5 py-0.5 rounded flex-shrink-0">{typeLabel}</span>}
                                  <span className={`flex-1 text-[13px] font-medium truncate ${item.status === "complete" ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{item.title}</span>
                                  ...
                                  {item.file_name ? (
                                    ...
                                    <button onClick={() => { setCloseoutUploadingId(item.id); closeoutFileRef.current?.click() }} disabled={closeoutUploadingId === item.id}
                                      className="text-[11px] text-[#8A9E94] hover:text-[#C9A84C] flex-shrink-0 hidden lg:block disabled:opacity-50">
                                  ...
                                  <div className="flex items-center gap-1 flex-shrink-0">
                                    <button onClick={() => { if (isEditing) { setCloseoutEditId(null); return } setCloseoutEditId(item.id); setCloseoutEditTitle(item.title); setCloseoutEditAssigned(item.assigned_to ?? ""); setCloseoutEditDue(item.due_date ?? ""); setCloseoutEditNotes(item.notes ?? "") }}
                                      className={`text-[11px] px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors ${isEditing ? "text-[#C9A84C]" : "text-[#8A9E94] hover:text-[#8A9E94]"}`}>
                                      {isEditing ? "Close" : "Edit"}
                                    </button>
                                    <button onClick={() => deleteCloseoutItem(item.id)} className="text-[11px] text-[#8A9E94] hover:text-red-400 px-2 py-1 rounded hover:bg-[#F0EDE4]/[0.05] transition-colors">Del</button>
                                  </div>
                                </div>
                                {isEditing && (
                                  <div className="px-4 pb-4 pt-2 border-t border-[#2A4F3F]/40 bg-[#0D1F1A]/30 space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">Title</label>
                                        <input value={closeoutEditTitle} onChange={e => setCloseoutEditTitle(e.target.value)} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40" /></div>
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">Status</label>
                                        <select value={item.status} onChange={e => updateCloseoutItem(item.id, { status: e.target.value })} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40">
                                      ...
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">Due Date</label>
                                        <input type="date" value={closeoutEditDue} onChange={e => setCloseoutEditDue(e.target.value)} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40" /></div>
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">Notes</label>
                                        <input value={closeoutEditNotes} onChange={e => setCloseoutEditNotes(e.target.value)} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40" /></div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <button onClick={() => { setCloseoutUploadingId(item.id); closeoutFileRef.current?.click() }} className="text-[11px] text-[#8A9E94] hover:text-[#C9A84C] px-2 py-1 border border-[#2A4F3F] rounded hover:border-[#2A4F3F] transition-colors">
                                        {item.file_name ? `attached: ${item.file_name.slice(0,25)}` : "+ Upload Document"}
                                      </button>
                                      <div className="flex-1" />
                                      <button onClick={() => setCloseoutEditId(null)} className="text-[12px] text-[#8A9E94] hover:text-[#8A9E94] px-3 py-1.5 rounded">Cancel</button>
                                      <button onClick={async () => { await updateCloseoutItem(item.id, { title: closeoutEditTitle, due_date: closeoutEditDue || null, notes: closeoutEditNotes || null }); setCloseoutEditId(null) }}
                                        className="text-[12px] font-semibold text-[#0D1F1A] bg-[#C9A84C] hover:bg-[#B8933D] px-4 py-1.5 rounded">Save</button>
                                    </div>
                                  </div>
                                )}
                              </div>
                          ...
                          {storedDocs.length === 0 && closeoutAllSubmittals.length === 0 && closeoutAllRFIs.length === 0 && closeoutAllCOs.length === 0 && closeoutAllDrawings.length === 0 && (
                            <div className="px-4 py-3 text-[12px] text-[#8A9E94] italic">No documents yet. Initialize the checklist or add project data.</div>
                          )}
                        </div>
                      )
                    })()}
                    ...
                    {CATS.filter(c => c.key !== "documents").map(cat => {
                      ...
                      return (
                        <div key={cat.key} className="bg-[#132820] rounded-xl border border-[#2A4F3F] overflow-hidden">
                          <div className="flex items-center justify-between px-4 py-3 border-b border-[#2A4F3F]">
                            <div className="flex items-center gap-2">
                              <span className="text-[13px] font-bold text-[#F0EDE4]">{cat.label}</span>
                              <span className="text-[11px] text-[#8A9E94]">{cat.done}/{cat.total} complete</span>
                              {cat.key === "financial" && closeoutTeam.length > 0 && <span className="text-[10px] text-[#8A9E94] bg-[#1C3B2E] px-1.5 py-0.5 rounded">{closeoutTeam.length} subs</span>}
                            </div>
                            <button onClick={() => { setNewCloseoutCategory(cat.key); setShowNewCloseout(true) }} className="text-[11px] text-[#8A9E94] hover:text-[#8A9E94] transition-colors flex items-center gap-1"><PlusIcon /> Add</button>
                          </div>

                          {items.length === 0 && (
                            <div className="px-4 py-3 text-[12px] text-[#8A9E94] italic">No items. Reinitialize or add manually.</div>
                          )}
                          {items.map(item => {
                            const isEditing = closeoutEditId === item.id
                              ...
                              <div key={item.id} className="border-b border-[#2A4F3F]/40 last:border-0">
                                <div className={`flex items-center gap-3 px-4 py-3 hover:bg-[#F0EDE4]/[0.02] transition-colors ${isEditing ? "bg-[#F0EDE4]/[0.02]" : ""}`}>
                                  ...
                                  {isWarranty && item.notes && (
                                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 bg-[#C9A84C]/20 text-[#C9A84C]">{item.notes}</span>
                                  )}
                                  <span className={`flex-1 text-[13px] font-medium truncate ${item.status === "complete" ? "text-[#8A9E94] line-through" : "text-[#F0EDE4]"}`}>{item.title}</span>
                                  {/* Trainer / date for training items */}
                                  {isTraining && item.assigned_to && (
                                    <span className="text-[11px] text-[#8A9E94] hidden md:block flex-shrink-0">{item.assigned_to}</span>
                                  )}
                                  {/* Expiry date for warranties */}
                                  {isWarranty && item.due_date && (
                                    <span className={`text-[11px] flex-shrink-0 hidden md:block ${new Date(item.due_date+"T00:00:00") < new Date() ? "text-red-400" : "text-[#8A9E94]"}`}>
                                ...
                                {isEditing && (
                                  <div className="px-4 pb-4 pt-2 border-t border-[#2A4F3F]/40 bg-[#0D1F1A]/30 space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                      ...
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">{isTraining ? "Trainer Name" : isWarranty ? "Warrantor" : "Assigned To"}</label>
                                        <input value={closeoutEditAssigned} onChange={e => setCloseoutEditAssigned(e.target.value)} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40" /></div>
                                      <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">{isTraining ? "Training Date" : isWarranty ? "Expiration Date" : "Due Date"}</label>
                                        <input type="date" value={closeoutEditDue} onChange={e => setCloseoutEditDue(e.target.value)} className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40" /></div>
                                    </div>
                                    <div><label className="block text-[11px] font-medium text-[#8A9E94] mb-1">Notes</label>
                                      <input value={closeoutEditNotes} onChange={e => setCloseoutEditNotes(e.target.value)} placeholder="Add notes…" className="w-full h-8 px-2 rounded border border-[#2A4F3F] bg-[#0D1F1A] text-[12px] text-[#F0EDE4] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 placeholder:text-[#8A9E94]" /></div>
                                    <div className="flex items-center gap-2">
                                      <button onClick={() => { setCloseoutUploadingId(item.id); closeoutFileRef.current?.click() }} className="text-[11px] text-[#8A9E94] hover:text-[#C9A84C] px-2 py-1 border border-[#2A4F3F] rounded hover:border-[#2A4F3F] transition-colors">
                                        {item.file_name ? `attached: ${item.file_name.slice(0,25)}` : "+ Upload Document"}
                                      </button>
                    ...
                    {openPunchCount === 0 && closeoutSubmittals.length === 0 && closeoutRFIs.length === 0 && closeoutCOs.length === 0 && closeoutDrawings.length === 0 && closeoutItems.every(i => i.status === "complete") && (
                      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-6 text-center">
                        ...
                        <p className="text-[13px] text-[#8A9E94] mt-1">All checklist items complete and no flagged items.</p>
                        <button disabled={closeoutGenerating}
                          ...
                          className="mt-4 h-9 px-5 rounded-lg bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2 disabled:opacity-50">
      ...
      {showNewCloseout && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) setShowNewCloseout(false) }}>
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[480px] p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Add Closeout Item</h2>
              <button onClick={() => setShowNewCloseout(false)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors"><XIcon className="h-4 w-4" /></button>
            </div>
            <div className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={() => setShowNewCloseout(false)} className="h-9 px-4 rounded-md text-[13px] text-[#8A9E94] hover:text-[#8A9E94] transition-colors">Cancel</button>
                <button onClick={addCloseoutItem} disabled={!newCloseoutTitle.trim()} className="h-9 px-5 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors disabled:opacity-50">Add Item</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {(showNewDaily || (viewDaily && dailyEditing)) && (() => {
        ...
        const tareaClass = "w-full px-3 py-2 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 resize-none placeholder:text-[#8A9E94]"
        return (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
            onClick={e => { if (e.target === e.currentTarget) onClose() }}>
            <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[680px] max-h-[90vh] flex flex-col">
              <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F] flex-shrink-0">
                <h2 className="text-[15px] font-bold text-[#F0EDE4]">{isEdit ? "Edit Daily Report" : "New Daily Report"}</h2>
                <button onClick={onClose} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors"><XIcon className="h-4 w-4" /></button>
              </div>
              <form onSubmit={isEdit ? saveDaily : createDaily} className="flex flex-col flex-1 min-h-0">
                <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <select value={dailyProjectId} onChange={e => setDailyProjectId(e.target.value)}
                        className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40">
                  ...
                  {!isEdit && (
                    <div>
                      <label className={labelCls}>Attachment <span className="text-[#8A9E94] font-normal">(optional)</span></label>
                      <input ref={dailyFileRef} type="file" className="w-full text-[12px] text-[#8A9E94] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#2A4F3F] file:bg-[#1C3B2E] file:text-[#8A9E94] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                    </div>
                  )}
                </div>
                <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#2A4F3F] flex-shrink-0">
                  <button type="button" onClick={onClose}
                    className="h-8 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors">
                    Cancel
                  </button>
                  <button type="submit" disabled={isEdit ? dailyEditSaving : dailySaving}
                    className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors disabled:opacity-50 flex items-center gap-2">
      ...
      {viewDaily && !dailyEditing && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[620px] max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F] flex-shrink-0">
              <div>
                <p className="text-[11px] text-[#8A9E94] uppercase tracking-widest font-bold">Daily Report</p>
                <h2 className="text-[16px] font-bold text-[#F0EDE4] mt-0.5">{fmtDateOnly(viewDaily.report_date)}</h2>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => openDailyForEdit(viewDaily)}
                  className="h-7 px-3 rounded-md border border-[#2A4F3F] text-[12px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors">
                  Edit
                </button>
                <button onClick={() => generateDailyPdf(viewDaily.id)} disabled={dailyGeneratingPdf}
                  className="h-7 px-3 rounded-md border border-[#2A4F3F] text-[12px] text-[#C9A84C] hover:bg-[#F0EDE4]/[0.05] transition-colors disabled:opacity-50 flex items-center gap-1.5">
                ...
                <button onClick={() => setViewDaily(null)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
            ...
            <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
              ...
              <div className="flex flex-wrap gap-4 text-[12px]">
                {viewDaily.prepared_by && <span><span className="text-[#8A9E94]">Prepared by: </span><span className="text-[#F0EDE4]">{viewDaily.prepared_by}</span></span>}
                {viewDaily.weather_conditions && <span><span className="text-[#8A9E94]">Weather: </span><span className="text-[#F0EDE4]">{viewDaily.weather_conditions}{viewDaily.temperature ? ` · ${viewDaily.temperature}` : ""}</span></span>}
                {viewDaily.manpower_count != null && <span><span className="text-[#8A9E94]">Manpower: </span><span className="text-[#F0EDE4]">{viewDaily.manpower_count} workers</span></span>}
              </div>
              {[
                ...
                <div key={f.label} className="rounded-md bg-[#132820] border border-[#2A4F3F] px-4 py-3">
                  <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-1.5">{f.label}</p>
                  <p className="text-[13px] text-[#F0EDE4] whitespace-pre-wrap">{f.value}</p>
                </div>
              ))}
              {viewDaily.file_name && (
                <div className="flex items-center gap-2 text-[12px] text-[#8A9E94] px-1">
      ...
      {(showNewDrawing || addRevisionFor) && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[560px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F]">
              <div>
                <h2 className="text-[15px] font-bold text-[#F0EDE4]">{addRevisionFor ? "Add Revision" : "Add Drawing"}</h2>
                {addRevisionFor && <p className="text-[12px] text-[#8A9E94] mt-0.5">Supersedes {addRevisionFor.drawing_number} Rev {addRevisionFor.revision}</p>}
              </div>
              <button onClick={() => { setShowNewDrawing(false); setAddRevisionFor(null) }} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
              ...
              <div className="px-6 py-4 space-y-3">
                ...
                <div>
                  ...
                  <textarea rows={2} value={dwgNotes} onChange={e => setDwgNotes(e.target.value)}
                    ...
                    className="w-full px-3 py-2 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 resize-none placeholder:text-[#8A9E94]" />
                </div>
                <div>
                  <label className={labelCls}>Attachment <span className="text-[#8A9E94] font-normal">(optional)</span></label>
                  <input ref={dwgFileRef} type="file" className="w-full text-[12px] text-[#8A9E94] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#2A4F3F] file:bg-[#1C3B2E] file:text-[#8A9E94] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                </div>
              </div>
              <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#2A4F3F]">
      ...
      {showNewPunch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[520px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F]">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">New Punch Item</h2>
              <button onClick={() => setShowNewPunch(false)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <form onSubmit={createPunch}>
              <div className="px-6 py-4 space-y-3">
                ...
                <div>
                  <label className={labelCls}>Attachment <span className="text-[#8A9E94] font-normal">(optional)</span></label>
                  <input ref={punchFileRef} type="file" className="w-full text-[12px] text-[#8A9E94] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#2A4F3F] file:bg-[#1C3B2E] file:text-[#8A9E94] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                </div>
              </div>
              <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#2A4F3F]">
      ...
      {viewPunch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[500px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F]">
              <div>
                <span className="text-[11px] font-mono text-[#C9A84C]">{viewPunch.item_number}</span>
                <h2 className="text-[15px] font-bold text-[#F0EDE4] mt-0.5">{viewPunch.description}</h2>
              </div>
              <button onClick={() => setViewPunch(null)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors ml-4 flex-shrink-0">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <div className="px-6 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-[12px]">
                {viewPunch.location && <div><span className="text-[#8A9E94]">Location: </span><span className="text-[#F0EDE4]">{viewPunch.location}</span></div>}
                {viewPunch.assigned_to && <div><span className="text-[#8A9E94]">Assigned to: </span><span className="text-[#F0EDE4]">{viewPunch.assigned_to}</span></div>}
                {viewPunch.due_date && <div><span className="text-[#8A9E94]">Due: </span><span className={new Date(viewPunch.due_date) < new Date() && viewPunch.status !== "Completed" ? "text-red-400 font-medium" : "text-[#F0EDE4]"}>{fmtDateOnly(viewPunch.due_date)}</span></div>}
                <div className="flex items-center gap-1.5"><span className="text-[#8A9E94]">Priority: </span><PunchPriorityBadge priority={viewPunch.priority} /></div>
              </div>
              {viewPunch.notes && (
                <div className="rounded-md bg-[#2A4F3F]/50 px-3 py-2">
                  <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest mb-1">Notes</p>
                  <p className="text-[13px] text-[#F0EDE4]">{viewPunch.notes}</p>
                </div>
              )}
              {viewPunch.file_name && (
                <div className="flex items-center gap-2 text-[12px] text-[#8A9E94]">
                  <svg className="h-3.5 w-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                  <span>{viewPunch.file_name}</span>
                </div>
              )}
              <div className="border-t border-[#2A4F3F] pt-4 space-y-3">
                <div>
            ...
            <div className="flex justify-between px-6 py-4 border-t border-[#2A4F3F]">
              <div className="flex gap-2">
                <button onClick={() => generatePunchPdf(viewPunch.id)} disabled={punchGeneratingPdf}
                  className="h-8 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors disabled:opacity-50 flex items-center gap-2">
      ...
      {showNewRfi && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[580px] flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F] flex-shrink-0">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">New RFI</h2>
              <button onClick={() => setShowNewRfi(false)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
              ...
              <div className="px-6 py-4 space-y-3 overflow-y-auto">
                ...
                <div>
                  ...
                  <input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                    ...
                    className="w-full text-[13px] text-[#8A9E94] file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-[12px] file:bg-[#2A4F3F] file:text-[#F0EDE4] hover:file:bg-[#2A4F3F]" />
                </div>
              </div>
              <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#2A4F3F] flex-shrink-0">
                <button type="button" onClick={() => setShowNewRfi(false)}
                  className="h-8 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors">Cancel</button>
      ...
      {viewRfi && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[680px] flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F] flex-shrink-0">
              <div className="flex items-center gap-3">
                <span className="text-[12px] font-mono text-[#C9A84C] flex-shrink-0">{viewRfi.rfi_number}</span>
                <h2 className="text-[15px] font-bold text-[#F0EDE4]">{viewRfi.subject}</h2>
                <RfiStatusBadge status={viewRfi.status} />
              </div>
              <button onClick={() => setViewRfi(null)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors ml-4 flex-shrink-0">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <div className="px-6 py-4 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="rounded-md bg-[#132820] px-3 py-2">
                    <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-0.5">{label}</p>
                    <p className="text-[12px] text-[#F0EDE4]">{value}</p>
                  </div>
                ))}
              </div>
              {/* Question */}
              {viewRfi.description && (
                <div className="rounded-md bg-[#2A4F3F]/50 px-3 py-2.5">
                  <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-1.5">Question</p>
                  <p className="text-[13px] text-[#F0EDE4] whitespace-pre-wrap">{viewRfi.description}</p>
                </div>
              )}
              {/* Attachment */}
              {viewRfi.file_name && (
                <div className="flex items-center gap-2 text-[12px]">
                  <svg className="w-4 h-4 text-[#8A9E94]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                  <span className="text-[#8A9E94]">{viewRfi.file_name}</span>
                </div>
              )}
              {viewRfi.generated_pdf_path && (
                <div className="flex items-center gap-2 text-[12px]">
                  <svg className="w-4 h-4 text-[#C9A84C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                  <button onClick={() => generateRfiPdf(viewRfi.id)} className="text-[#C9A84C] hover:text-[#D4B56A] transition-colors">View / Regenerate PDF</button>
                </div>
              )}
              {/* Response */}
              <div className="border-t border-[#2A4F3F] pt-4 space-y-3">
                <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest">Response</p>
            ...
            <div className="flex justify-between px-6 py-4 border-t border-[#2A4F3F] flex-shrink-0">
              <div className="flex gap-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setViewRfi(null)}
                  className="h-8 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors">Close</button>
      ...
      {showNewCo && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#132820] border border-[#2A4F3F] rounded-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#2A4F3F] flex-shrink-0">
              <h2 className="text-[16px] font-bold text-[#F0EDE4]">New Change Order</h2>
              <button onClick={() => setShowNewCo(false)} className="text-[#8A9E94] hover:text-[#F0EDE4] transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <form onSubmit={createCo} className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
              {(() => {
                const labelCls2 = "block text-[11px] font-semibold text-[#8A9E94] uppercase tracking-widest mb-1"
                const inputCls2 = "w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 placeholder:text-[#8A9E94]"
                const selCls2   = "w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40"
                ...
                return (<>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      {coProj && (
                        <p className="text-[11px] text-[#8A9E94] mt-1">{[coProj.gc_name, coProj.location].filter(Boolean).join(" · ")}</p>
                      )}
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input type="file" accept=".pdf,.doc,.docx,.xlsx,.xls,.png,.jpg,.jpeg"
                      ...
                      className="w-full text-[13px] text-[#8A9E94] file:mr-3 file:py-1.5 file:px-3 file:rounded file:border-0 file:bg-[#2A4F3F] file:text-[#F0EDE4] file:text-[12px] file:cursor-pointer hover:file:bg-[#2A4F3F] cursor-pointer" />
                  </div>
                </>)
              })()}
            </form>
      ...
      {viewCo && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#132820] border border-[#2A4F3F] rounded-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
            {/* Header */}
            <div className="flex items-start justify-between px-6 py-4 border-b border-[#2A4F3F] flex-shrink-0">
              <div>
                <div className="flex items-center gap-3">
                  <h2 className="text-[16px] font-bold text-[#F0EDE4]">{viewCo.co_number}</h2>
                  {(() => {
                    const statusColor: Record<string, string> = {
                      Draft: "bg-[#2A4F3F] text-[#8A9E94]",
                      Submitted: "bg-[#C9A84C]/15 text-[#C9A84C]",
                    ...
                    return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${statusColor[viewCo.status] ?? "bg-[#2A4F3F] text-[#8A9E94]"}`}>{viewCo.status}</span>
                  })()}
                </div>
                <p className="text-[12px] text-[#8A9E94] mt-0.5">{viewCo.date ? fmtDateOnly(viewCo.date) : "No date"}</p>
              </div>
              <button onClick={() => setViewCo(null)} className="text-[#8A9E94] hover:text-[#F0EDE4] transition-colors mt-0.5">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-5">
              ...
              {viewCo.project_id && (() => {
                ...
                if (!proj) return null
                return (
                  <div className="rounded-lg bg-[#132820] border border-[#2A4F3F] px-4 py-3 grid grid-cols-2 gap-x-6 gap-y-1.5 text-[12px]">
                    {proj.name    && <div><span className="text-[#8A9E94]">Project: </span><span className="text-[#F0EDE4] font-medium">{proj.name}</span></div>}
                    {proj.number  && <div><span className="text-[#8A9E94]">No.: </span><span className="text-[#F0EDE4]">{proj.number}</span></div>}
                    {proj.gc_name && <div><span className="text-[#8A9E94]">GC: </span><span className="text-[#F0EDE4]">{proj.gc_name}</span></div>}
                    {proj.architect && <div><span className="text-[#8A9E94]">Architect: </span><span className="text-[#F0EDE4]">{proj.architect}</span></div>}
                    {proj.location && <div className="col-span-2"><span className="text-[#8A9E94]">Location: </span><span className="text-[#F0EDE4]">{proj.location}</span></div>}
                  </div>
                )
              })()}
              ...
              <div className="grid grid-cols-3 gap-3 text-[12px]">
                {[
                  ...
                  <div key={label}>
                    <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-0.5">{label}</p>
                    <p className="text-[#F0EDE4]">{value}</p>
                  </div>
                ))}
              </div>

              {/* Pricing */}
              <div className="rounded-lg bg-[#132820] border border-[#2A4F3F] px-4 py-3">
                <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-1">Total Change Order Amount</p>
                <p className={`text-[22px] font-bold tabular-nums ${viewCo.status === "Approved" ? "text-green-400" : "text-[#F0EDE4]"}`}>
              ...
              {viewCo.proposal && (
                <div>
                  <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest mb-2">Proposal</p>
                  <p className="text-[13px] text-[#F0EDE4] whitespace-pre-wrap leading-relaxed">{viewCo.proposal}</p>
                </div>
              )}
              ...
              {viewCo.qualifications && (
                <div>
                  <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest mb-2">Qualifications / Exclusions</p>
                  <p className="text-[13px] text-[#F0EDE4] whitespace-pre-wrap leading-relaxed">{viewCo.qualifications}</p>
                </div>
              )}
              ...
              {viewCo.file_name && (
                <div className="flex items-center gap-2 text-[12px]">
                  <svg className="w-4 h-4 text-[#8A9E94]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                  <span className="text-[#8A9E94]">{viewCo.file_name}</span>
                </div>
              )}
              {viewCo.generated_pdf_path && (
                <div className="flex items-center gap-2 text-[12px]">
                  <svg className="w-4 h-4 text-[#C9A84C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                  <button onClick={() => generateCoPdf(viewCo.id)} className="text-[#C9A84C] hover:text-[#D4B56A] transition-colors">View / Regenerate PDF</button>
                </div>
              )}

              {/* Status update */}
              <div className="border-t border-[#2A4F3F] pt-4 space-y-3">
                <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest">Update</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[11px] font-semibold text-[#8A9E94] uppercase tracking-widest mb-1">Status</label>
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-[#8A9E94] uppercase tracking-widest mb-1">Assigned To</label>
                    <input type="text" value={coAssignedTo} onChange={e => setCoAssignedTo(e.target.value)}
                      placeholder="Reviewer name"
                      className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 placeholder:text-[#8A9E94]" />
                  </div>
                </div>
              </div>
            </div>
      ...
      {openFileCtx && fileModalStep === "project" && (
        ...
        >
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[460px] p-6">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Open Submittal</h2>
              <button onClick={closeFileModal} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <p className="text-[12px] text-[#8A9E94] mb-4 truncate">{openFileCtx.file.file_name}</p>

            <div className="mb-4">
              <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Which project is this for?</label>
              <select
            ...
            <div className="flex justify-between items-center gap-2">
              <button
                ...
                className="h-8 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors"
              >
                Cancel
              </button>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors"
                >
                  Continue →
                </button>
              </div>
      ...
      {openFileCtx && fileModalStep === "coversheet" && (
        ...
        >
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[460px] p-6">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Add Cover Sheet?</h2>
              <button onClick={closeFileModal} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <p className="text-[12px] font-semibold text-[#F0EDE4] mb-1">
              {modalProjectId ? (appProjects.find(p => p.id === modalProjectId)?.name ?? "Project") : "No project selected"}
            </p>
            <p className="text-[13px] text-[#8A9E94] mb-5">
              Generate a submittal transmittal cover sheet and merge it with this document.
            </p>
            <div className="flex flex-col gap-2">
              <button
                onClick={initCoverForm}
                className="h-9 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors"
              >
                Yes, add cover sheet
              </button>
              <button
                onClick={openFileDirectly}
                className="h-9 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors"
              >
                No, just open
              </button>
              <button
      ...
      {openFileCtx && fileModalStep === "form" && coverForm && (
        <div
          ...
          onClick={e => { if (e.target === e.currentTarget) closeFileModal() }}
        >
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[680px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F]">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Submittal Transmittal</h2>
              <button onClick={closeFileModal} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
              ...
              <div className="px-6 py-4 space-y-3 overflow-y-auto max-h-[75vh]">
                ...
                <div className="flex gap-3">
                  <div className="flex-1">
                    ...
                    <select value={coverForm.reviewedBy} onChange={e => setCoverForm(prev => ({ ...prev!, reviewedBy: e.target.value }))} className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40">
                      <option value="">Select…</option>
                  ...
                  <div className="flex-1">
                    ...
                    <select value={coverForm.certifiedBy} onChange={e => setCoverForm(prev => ({ ...prev!, certifiedBy: e.target.value }))} className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40">
                      <option value="">Select…</option>
                      {teamMembers.map(m => <option key={m.id} value={m.name}>{m.name}{m.title ? ` — ${m.title}` : ""}</option>)}
                    </select>
                  </div>
      ...
      {showUpload && (
        ...
        >
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[440px] p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Upload Submittal</h2>
              <button onClick={closeModal} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleUpload} className="space-y-3">
              {/* File */}
              <div>
                <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">File</label>
                <input
                  type="file"
                  ...
                  required
                  className="w-full text-[13px] text-[#F0EDE4] file:mr-3 file:text-[12px] file:bg-[#2A4F3F] file:border-0 file:rounded-md file:px-3 file:py-1.5 file:text-[#F0EDE4] file:cursor-pointer cursor-pointer disabled:opacity-50"
                />
              </div>

              {/* Classifying spinner */}
              {uploadStep === "classifying" && (
                <div className="flex items-center gap-2 py-1 text-[13px] text-[#8A9E94]">
              ...
              {uploadStep === "suggested" && aiResult && (
                <div className="rounded-lg border border-[#C9A84C]/30 bg-[#C9A84C]/10 p-3 space-y-2">
                  <p className="text-[11px] font-bold text-[#C9A84C] uppercase tracking-widest">✦ AI Suggestion</p>
                  <div>
                    <p className="text-[13px] font-semibold text-[#F0EDE4]">{aiResult.division_num} — {aiResult.division_name}</p>
                    <p className="text-[12px] text-[#8A9E94] mt-0.5">{aiResult.section_code} — {aiResult.section_name}</p>
                  </div>
                  {(aiResult.material_name || aiResult.manufacturer || aiResult.dimensions) && (
                    <div className="border-t border-[#C9A84C]/20 pt-2 space-y-0.5">
                      {aiResult.material_name && <p className="text-[12px] text-[#F0EDE4]"><span className="text-[#8A9E94]">Material:</span> {aiResult.material_name}</p>}
                      {aiResult.manufacturer  && <p className="text-[12px] text-[#F0EDE4]"><span className="text-[#8A9E94]">Mfr:</span> {aiResult.manufacturer}</p>}
                      {aiResult.dimensions    && <p className="text-[12px] text-[#F0EDE4]"><span className="text-[#8A9E94]">Dims:</span> {aiResult.dimensions}</p>}
                    </div>
                  )}
                  {aiResult.confidence != null && (
                    <div className="border-t border-[#C9A84C]/20 pt-2 flex items-center gap-2">
                      <div className="flex-1 h-1 rounded-full bg-[#2A4F3F] overflow-hidden">
                  ...
                  {aiResult.reasoning && <p className="text-[11px] text-[#8A9E94] italic">{aiResult.reasoning}</p>}
                  <div className="flex gap-2 pt-0.5">
                    <button type="button" onClick={acceptSuggestion}
                      className="h-7 px-3 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[12px] font-semibold hover:bg-[#B8933D] transition-colors">
                      Use this →
                    </button>
              ...
              {uploadStep === "manual" && (
                <>
                  <div>
                    <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Division</label>
                    <select
                      value={uploadDiv}
                      onChange={e => {
                      ...
                      required
                      className="w-full h-9 px-2 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40"
                    >
                  ...
                  <div>
                    <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Section</label>
                    <select
                      value={uploadSec}
                      onChange={e => {
                      ...
                      disabled={!uploadDiv}
                      className="w-full h-9 px-2 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
              ...
              {uploadStep === "naming" && (
                <div className="space-y-3 pt-1">
                  <div className="h-px bg-[#2A4F3F]" />
                  <p className="text-[11px] font-bold text-[#8A9E94] uppercase tracking-widest">Submittal Name</p>

                  <div>
                    <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Material</label>
                    <Combobox value={nameMatl} onChange={setNameMatl} options={nameOpts.materials} placeholder="e.g. Gypsum Board" autoFocus />
                  </div>

                  <div>
                    <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Manufacturer</label>
                    <Combobox value={nameMfr} onChange={setNameMfr} options={nameOpts.manufacturers} placeholder="e.g. Georgia-Pacific" />
                  </div>

                  <div>
                    <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Dimensions</label>
                    <Combobox value={nameDims} onChange={setNameDims} options={nameOpts.dimensions} placeholder='e.g. 5/8" x 4&apos; x 8&apos;' />
                  </div>

                  {(nameMatl || nameMfr || nameDims) && (
                    <div className="rounded-md bg-[#C9A84C]/10 border border-[#C9A84C]/20 px-3 py-2">
                      <p className="text-[10px] font-bold text-[#C9A84C] uppercase tracking-widest mb-0.5">Name preview</p>
                      <p className="text-[13px] font-medium text-[#F0EDE4] truncate">
      ...
      {editSubmittal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[460px] p-6">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Edit Submittal</h2>
              <button onClick={() => setEditSubmittal(null)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors"><XIcon className="h-4 w-4" /></button>
            </div>
            <p className="text-[12px] text-[#8A9E94] mb-4 truncate">{editSubmittal.file_name}</p>

            <div className="space-y-3">
              <div>
                <label className="block text-[12px] font-medium text-[#8A9E94] mb-1">Status</label>
              ...
              <div>
                ...
                }} className="w-full h-9 px-3 rounded-md border border-[#2A4F3F] text-[13px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 disabled:opacity-50">
              ...
              {editSubmittal.ai_reasoning && (
                <div className="rounded-md bg-[#2A4F3F]/50 px-3 py-2">
                  <p className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest mb-0.5">AI Reasoning</p>
                  <p className="text-[12px] text-[#8A9E94] italic">{editSubmittal.ai_reasoning}</p>
                </div>
              )}
            </div>
      ...
      {showBatch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[700px] max-h-[85vh] flex flex-col">

            {/* Header */}
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#2A4F3F] flex-shrink-0">
              <div>
                <h2 className="text-[15px] font-bold text-[#F0EDE4]">Batch Upload</h2>
                <p className="text-[12px] text-[#8A9E94] mt-0.5">AI will classify each file — review before uploading</p>
              </div>
              <button onClick={closeBatch} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 min-h-0">
              ...
              {batchPhase === "select" && (
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-10 text-center transition-colors ${batchDragOver ? "border-[#C9A84C]/60 bg-[#C9A84C]/5" : "border-[#2A4F3F]"}`}
                >
                  <div className="w-12 h-12 rounded-xl bg-[#2A4F3F] flex items-center justify-center mx-auto mb-3">
                    <LayersIcon />
                  </div>
                  <p className="text-[14px] font-semibold text-[#F0EDE4] mb-1">Drop files here</p>
                  <p className="text-[12px] text-[#8A9E94] mb-4">PDF, DOC, DOCX, XLS, XLSX, DWG, RVT</p>
                  <label className="cursor-pointer h-8 px-4 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors inline-flex items-center gap-2">
                  ...
                  {batchItems.length > 0 && (
                    <p className="mt-4 text-[13px] text-[#C9A84C]">{batchItems.length} file{batchItems.length !== 1 ? "s" : ""} selected</p>
                  )}
                </div>
              )}
              ...
              {(batchPhase === "classifying" || batchPhase === "review" || batchPhase === "uploading" || batchPhase === "done") && (
                <div className="space-y-1.5">
                  ...
                  <div className="grid gap-2 px-2 pb-1" style={{ gridTemplateColumns: "1fr 155px 195px 20px 20px 20px" }}>
                    <span className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">File</span>
                    <span className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Division</span>
                    <span className="text-[10px] font-bold text-[#8A9E94] uppercase tracking-widest">Section</span>
                    <span /><span /><span />
                  </div>
                  {batchItems.map(item => {
                    ...
                    return (
                      <div key={item.id} className="bg-[#132820] rounded-lg overflow-hidden mb-1">
                        {/* Main row */}
                        <div className="grid gap-2 items-center px-2 py-1.5" style={{ gridTemplateColumns: "1fr 155px 195px 20px 20px 20px" }}>
                          <div className="min-w-0 flex flex-col gap-0.5">
                            {isEditable ? (
                              <input
                                ...
                                className="h-6 px-1.5 rounded border border-[#2A4F3F] text-[11px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none focus:ring-1 focus:ring-[#C9A84C]/40 w-full placeholder:text-[#8A9E94]"
                              />
                            ) : (
                              <span className="text-[12px] text-[#F0EDE4] truncate" title={item.file.name}>{item.customName || item.file.name}</span>
                            )}
                            <span className="text-[10px] text-[#8A9E94] truncate" title={item.file.name}>{item.file.name}</span>
                          </div>
                          ...
                          <select value={item.divNum} disabled={!isEditable}
                            ...
                            className="h-7 px-1.5 rounded-md border border-[#2A4F3F] text-[11px] text-[#F0EDE4] bg-[#0D1F1A] focus:outline-none disabled:opacity-60 w-full">
                          ...
                          <div className="flex items-center justify-center">
                            ...
                            {item.status === "pending"      && <span className="w-2 h-2 rounded-full bg-[#2A4F3F]" />}
                          ...
                          {isEditable && (
                            <button type="button" onClick={() => updateBatchItem(item.id, { expanded: !item.expanded })}
                              ...
                              className={`flex items-center justify-center transition-colors ${item.expanded || hasName ? "text-[#C9A84C]" : "text-[#8A9E94] hover:text-[#8A9E94]"}`}>
                          ...
                          {isEditable && (
                            <button type="button" onClick={() => setBatchItems(prev => prev.filter(it => it.id !== item.id))}
                              className="text-[#8A9E94] hover:text-red-400 transition-colors flex items-center justify-center">
                        ...
                        {item.expanded && isEditable && (
                          <div className="border-t border-[#2A4F3F] px-2 pb-2 pt-2 grid grid-cols-3 gap-2">
                            <div>
                              <label className="block text-[10px] font-medium text-[#8A9E94] mb-1">Material</label>
                            ...
                            <div>
                              <label className="block text-[10px] font-medium text-[#8A9E94] mb-1">Manufacturer</label>
                            ...
                            <div>
                              <label className="block text-[10px] font-medium text-[#8A9E94] mb-1">Dimensions</label>
                            ...
                            {hasName && (
                              <div className="col-span-3 text-[11px] text-[#C9A84C] truncate">
                  ...
                  {batchPhase === "classifying" && (
                    <p className="text-[12px] text-[#8A9E94] pt-2 text-center">
                  ...
                  {batchPhase === "done" && (() => {
                    ...
                    return (
                      <div className="mt-3 rounded-lg border border-[#2A4F3F] bg-[#132820] px-4 py-3 text-center">
                        <p className="text-[13px] font-semibold text-[#F0EDE4]">
            ...
            {/* Footer */}
            <div className="flex-shrink-0 border-t border-[#2A4F3F] px-6 py-4 flex justify-between items-center">
              <button onClick={closeBatch} className="h-8 px-4 rounded-md border border-[#2A4F3F] text-[13px] text-[#8A9E94] hover:bg-[#F0EDE4]/[0.05] transition-colors">
              ...
              <div className="flex items-center gap-3">
                ...
                {batchPhase === "select" && (
                  <button
                    ...
                    className="h-8 px-5 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors disabled:opacity-40 flex items-center gap-2"
                  >
                ...
                {batchPhase === "review" && (
                  <button
                    ...
                    onClick={uploadBatch}
                    className="h-8 px-5 rounded-md bg-[#C9A84C] text-[#0D1F1A] text-[13px] font-semibold hover:bg-[#B8933D] transition-colors disabled:opacity-40"
                  >
                    Upload {batchItems.filter(it => it.divNum && it.secCode).length} files
                  </button>
                )}
                {batchPhase === "uploading" && (
                  <div className="flex items-center gap-2 text-[13px] text-[#8A9E94]">
      ...
      {showManage && (
        ...
        >
          <div className="bg-[#132820] rounded-xl border border-[#2A4F3F] shadow-2xl w-[360px] p-5">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-[15px] font-bold text-[#F0EDE4]">Manage Divisions</h2>
              <button onClick={() => setShowManage(false)} className="text-[#8A9E94] hover:text-[#8A9E94] transition-colors">
                <XIcon className="h-4 w-4" />
              </button>
            </div>
            <p className="text-[12px] text-[#8A9E94] mb-3">Uncheck divisions to hide them from the sidebar.</p>
              ...
              {CSI_DIVISIONS.map(d => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 h-8 px-2 rounded-md hover:bg-[#F0EDE4]/[0.05] transition-colors text-left"
                  >
                    <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${hidden ? "border-[#2A4F3F] bg-transparent" : "border-[#C9A84C] bg-[#C9A84C]"}`}>
                      {!hidden && <CheckIcon />}
                    </span>
                    <span className="text-[11px] font-mono text-[#8A9E94] w-5 text-right flex-shrink-0">{d.num}</span>
                    <span className={`text-[13px] truncate transition-colors ${hidden ? "text-[#8A9E94]" : "text-[#F0EDE4]"}`}>{d.name}</span>
                  </button>
                )
              })}
            </div>
