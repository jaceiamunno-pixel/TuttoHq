...
function StatusBadge({ status }: { status: string }) {
  const cls = STATUS_STYLES[status] ?? "bg-[#F1F3F5] text-[#64748B]"
...
const RFI_STATUS_STYLES: Record<string, string> = {
  ...
  "Void":         "bg-[#F1F3F5] text-[#64748B]",
}
function RfiStatusBadge({ status }: { status: string }) {
  const cls = RFI_STATUS_STYLES[status] ?? "bg-[#F1F3F5] text-[#64748B]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
...
const PUNCH_PRIORITY_STYLES: Record<string, string> = {
  "Low":      "bg-[#F1F3F5] text-[#64748B]",
  "Medium":   "bg-[#7B9BB5]/15 text-[#7B9BB5]",
  "High":     "bg-amber-100 text-amber-700",
  "Critical": "bg-red-100 text-red-700",
}
function PunchStatusBadge({ status }: { status: string }) {
  const cls = PUNCH_STATUS_STYLES[status] ?? "bg-[#F1F3F5] text-[#64748B]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
function PunchPriorityBadge({ priority }: { priority: string }) {
  const cls = PUNCH_PRIORITY_STYLES[priority] ?? "bg-[#F1F3F5] text-[#64748B]"
...
const DRAWING_STATUS_STYLES: Record<string, string> = {
  ...
  "Superseded":              "bg-[#F1F3F5] text-[#64748B]",
  "Void":                    "bg-red-100 text-red-700",
}
function DrawingStatusBadge({ status }: { status: string }) {
  const cls = DRAWING_STATUS_STYLES[status] ?? "bg-[#F1F3F5] text-[#64748B]"
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${cls}`}>{status}</span>
}
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-[#F8F9FA] border border-[#E2E8F0] rounded-md shadow-xl max-h-44 overflow-y-auto">
          {filtered.map(opt => (
            <button
              key={opt}
              type="button"
...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <aside className={`fixed left-0 top-0 h-screen z-40 bg-[#F8F9FA] border-r border-[#E2E8F0] flex flex-col overflow-hidden transition-[width] duration-200 ease-in-out ${sidebarOpen ? "w-80" : "w-12"}`}>
          ...
          <form onSubmit={handleSearch}>
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-8 pl-8 pr-6 rounded-md text-[13px] bg-white border border-[#E2E8F0] text-[#0F172A] placeholder-[#64748B] focus:outline-none focus:ring-1 focus:ring-[#7B9BB5]/40 focus:border-[#7B9BB5]/50 transition-all"
              />
              {query && (
                <button
                  type="button"
      ...
      <div className="flex-1 flex flex-col min-h-0 ml-12">
        ...
        {/* Logo bar */}
        {logoUrl && (
          <div className="flex-shrink-0 border-b border-[#E2E8F0] flex items-center justify-end px-6 py-2 bg-[#F8F9FA]">
            ...
            <div className="ml-auto flex items-center gap-2 py-1.5 flex-shrink-0">
              ...
              <div className="relative">
                <select
                  ...
                  className="h-7 pl-3 pr-7 rounded-md border border-white/30 bg-white/20 text-[12px] text-white appearance-none cursor-pointer hover:bg-white/30 transition-colors focus:outline-none focus:border-white backdrop-blur-sm"
                >
        ...
        <div className="flex-shrink-0 border-b border-[#E2E8F0] bg-[#F8F9FA] flex items-center justify-between px-4 py-2.5">
                  ...
                  return (
                    <div className="flex items-stretch gap-3 px-4 py-3 border-b border-[#E2E8F0] flex-shrink-0">
                      {[
                        ...
                        <div key={label} className="flex-1 rounded-lg bg-[#F1F3F5] border border-[#E2E8F0] px-3 py-2">
            ...
            <table className="w-full text-[13px] border-collapse">
                    ...
                    <tbody>
                      {changeOrders.map(c => {
                        ...
                        const badgeCls = statusColor[c.status] ?? "bg-[#F1F3F5] text-[#64748B]"
                        return (
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      {CATS.map(cat => {
                        ...
                        return (
                          <div key={cat.key} className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] p-3">
                            ...
                            return (
                              <div key={item.id} className="border-b border-[#E2E8F0]/40 last:border-0">
                                <div className={`flex items-center gap-3 px-4 py-3 hover:bg-[#0F172A]/[0.02] transition-colors ${isEditing ? "bg-[#0F172A]/[0.02]" : ""}`}>
                                  ...
                                  {typeLabel && <span className="text-[10px] font-bold text-[#64748B] bg-[#F1F3F5] px-1.5 py-0.5 rounded flex-shrink-0">{typeLabel}</span>}
                    ...
                    {CATS.filter(c => c.key !== "documents").map(cat => {
                      ...
                      return (
                        <div key={cat.key} className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] overflow-hidden">
                          <div className="flex items-center justify-between px-4 py-3 border-b border-[#E2E8F0]">
                            <div className="flex items-center gap-2">
                              ...
                              {cat.key === "financial" && closeoutTeam.length > 0 && <span className="text-[10px] text-[#64748B] bg-[#F1F3F5] px-1.5 py-0.5 rounded">{closeoutTeam.length} subs</span>}
                            </div>
      ...
      {showNewCloseout && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center" onClick={e => { if (e.target === e.currentTarget) setShowNewCloseout(false) }}>
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[480px] p-6">
      ...
      {(showNewDaily || (viewDaily && dailyEditing)) && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
            ...
            <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[680px] max-h-[90vh] flex flex-col">
                ...
                <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
                  ...
                  {!isEdit && (
                    <div>
                      ...
                      <input ref={dailyFileRef} type="file" className="w-full text-[12px] text-[#64748B] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#E2E8F0] file:bg-[#F1F3F5] file:text-[#64748B] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                    </div>
                  )}
                </div>
      ...
      {viewDaily && !dailyEditing && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[620px] max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0] flex-shrink-0">
              <div>
            ...
            <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
              ...
              {[
                ...
                <div key={f.label} className="rounded-md bg-[#F8F9FA] border border-[#E2E8F0] px-4 py-3">
      ...
      {(showNewDrawing || addRevisionFor) && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[560px]">
            ...
            <form onSubmit={createDrawing}>
              <div className="px-6 py-4 space-y-3">
                ...
                <div>
                  <label className={labelCls}>Attachment <span className="text-[#64748B] font-normal">(optional)</span></label>
                  <input ref={dwgFileRef} type="file" className="w-full text-[12px] text-[#64748B] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#E2E8F0] file:bg-[#F1F3F5] file:text-[#64748B] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                </div>
              </div>
      ...
      {showNewPunch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[520px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0]">
            ...
            <form onSubmit={createPunch}>
              <div className="px-6 py-4 space-y-3">
                ...
                <div>
                  <label className={labelCls}>Attachment <span className="text-[#64748B] font-normal">(optional)</span></label>
                  <input ref={punchFileRef} type="file" className="w-full text-[12px] text-[#64748B] file:mr-3 file:py-1 file:px-3 file:rounded file:border file:border-[#E2E8F0] file:bg-[#F1F3F5] file:text-[#64748B] file:text-[11px] file:cursor-pointer hover:file:bg-white/[0.05]" />
                </div>
              </div>
              <div className="flex justify-end gap-2 px-6 py-4 border-t border-[#E2E8F0]">
      ...
      {viewPunch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[500px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0]">
              <div>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              {viewPunch.notes && (
                <div className="rounded-md bg-[#F1F3F5] px-3 py-2">
      ...
      {showNewRfi && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[580px] flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0] flex-shrink-0">
      ...
      {viewRfi && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[680px] flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0] flex-shrink-0">
              <div className="flex items-center gap-3">
            ...
            <div className="px-6 py-4 space-y-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="rounded-md bg-[#F8F9FA] px-3 py-2">
              ...
              {/* Question */}
              {viewRfi.description && (
                <div className="rounded-md bg-[#F1F3F5] px-3 py-2.5">
      ...
      {showNewCo && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[#F8F9FA] border border-[#E2E8F0] rounded-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
          ...
          <div className="bg-[#F8F9FA] border border-[#E2E8F0] rounded-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  {(() => {
                    ...
                    return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${statusColor[viewCo.status] ?? "bg-[#F1F3F5] text-[#64748B]"}`}>{viewCo.status}</span>
                  })()}
                </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-5">
              ...
              {viewCo.project_id && (() => {
                ...
                if (!proj) return null
                return (
                  <div className="rounded-lg bg-[#F8F9FA] border border-[#E2E8F0] px-4 py-3 grid grid-cols-2 gap-x-6 gap-y-1.5 text-[12px]">
              ...
              {/* Pricing */}
              <div className="rounded-lg bg-[#F8F9FA] border border-[#E2E8F0] px-4 py-3">
      ...
      {openFileCtx && fileModalStep === "project" && (
        ...
        >
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[460px] p-6">
      ...
      {openFileCtx && fileModalStep === "form" && coverForm && (
        <div
          ...
          onClick={e => { if (e.target === e.currentTarget) closeFileModal() }}
        >
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[680px]">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#E2E8F0]">
      ...
      {showUpload && (
        ...
        >
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[440px] p-6">
      ...
      {showBatch && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"
          ...
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[700px] max-h-[85vh] flex flex-col">
              ...
              {(batchPhase === "classifying" || batchPhase === "review" || batchPhase === "uploading" || batchPhase === "done") && (
                <div className="space-y-1.5">
                  ...
                  {batchItems.map(item => {
                    ...
                    return (
                      <div key={item.id} className="bg-[#F8F9FA] rounded-lg overflow-hidden mb-1">
                        {/* Main row */}
                  ...
                  {batchPhase === "done" && (() => {
                    ...
                    return (
                      <div className="mt-3 rounded-lg border border-[#E2E8F0] bg-[#F8F9FA] px-4 py-3 text-center">
      ...
      {showManage && (
        ...
        >
          <div className="bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] shadow-2xl w-[360px] p-5">
            <div className="flex items-center justify-between mb-1">
