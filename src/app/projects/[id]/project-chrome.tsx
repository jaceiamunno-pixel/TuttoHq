...
const PROJECT_NAV: { slug: string; label: string; icon: React.ReactNode }[] = [
  ...
  { slug: "punch",           label: "Punch List",    icon: <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg> },
...
function ProjectSwitcher({ projects, currentId, onPick }: {
  ...
  const matches = q ? projects.filter(p => `${p.name} ${p.number ?? ""}`.toLowerCase().includes(q)) : projects
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            ...
            {matches.map(p => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-[12px] truncate transition-colors ${p.id === currentId ? "bg-[#7B9BB5]/20 text-white" : "text-[#94A3B8] hover:bg-white/[0.06] hover:text-white"}`}
              >
