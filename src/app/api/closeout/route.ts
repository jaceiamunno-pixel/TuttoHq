...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    supabase.from("submittals").select("id,file_name,csi_section,csi_division,division_name,section_name,review_status,created_at").eq("project_id", projectId).eq("status", "active").is("deleted_at", null).order("csi_section"),
    ...
    supabase.from("submittals").select("id,file_name,review_status").eq("project_id", projectId).eq("status", "active").is("deleted_at", null).not("review_status", "eq", "Approved"),
